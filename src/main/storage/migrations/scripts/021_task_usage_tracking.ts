/**
 * Migration 021: Task Usage Tracking
 *
 * Adds total_usage column to tasks table for tracking token usage across all iterations.
 */
...
export const migration: Migration = {
  ...
  up(db: Database): void {
    ...
    if (!taskColumnNames.includes('total_usage')) {
      db.exec(`ALTER TABLE tasks ADD COLUMN total_usage TEXT DEFAULT NULL`)
    }
  },

  down(db: Database): void {
    // Recreate tasks table without total_usage
    db.exec(`
    ...
    // Copy data (excluding total_usage)
    db.exec(`
      ...
      SELECT
        ...
        external_issue_id, external_issue_url, integration_id, tag_ids, session_id
      FROM tasks
    `)
